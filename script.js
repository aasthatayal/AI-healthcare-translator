class HealthcareTranslator {
    ...
    async loadApiKey() {
        // For client-side apps, we need to handle API keys differently
        // In production, this should use a backend API endpoint for security
        
        // Try to get from meta tag (injected during build)
        const metaApiKey = document.querySelector('meta[name="gemini-api-key"]');
        if (metaApiKey) {
            this.geminiApiKey = metaApiKey.getAttribute('content');
        }
        
        // For demo purposes, use mock translations if no API key
    ...
    initializeSpeechRecognition() {
        ...
        this.recognition.onresult = (event) => {
            ...
            // Translate if we have final results
    ...
    async callGeminiAPI(text, sourceLang, targetLang) {
        ...
        // Use mock translation for demo mode
    ...
    getMockTranslation(text, targetLang) {
        // Enhanced mock translation for demo purposes
        ...
        // Return text with language indicator for unmatched text
    ...
    insertPhrase(phrase) {
        ...
        // Add phrase to original transcript
    ...
    handleKeyboardShortcuts(e) {
        // Ctrl/Cmd + Space: Start/Stop listening
        ...
        // Escape: Stop listening and clear
        if (e.code === 'Escape') {
            this.stopListening();
            this.synthesis.cancel();
        }
        ...
        // Ctrl/Cmd + Delete: Clear transcripts
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize the healthcare translator
    const translator = new HealthcareTranslator();
    
    // Make translator globally available for debugging
